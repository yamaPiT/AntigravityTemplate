...
-   **ドキュメントの絶対的最新化ルール (Single Source of Truth)**: 実装やテストの工程において機能追加やバグ修正による設計変更が必要と判断された場合、エージェントは「コードだけを先行して修正する」ことを固く禁じられています。必ず先立ってアーキテクチャ設計工程（ステップ2）等へ戻り、設計書を最新状態に更新した上で、再びコード実装・テストを進めます。
