...
graph TD
    ...
    ArchRev -- 修正・洗練 --> Arch
    ArchRev --> ArchHum["人間によるレビュー<br>(大幅変更時のみ)"]
    ArchHum -- 修正依頼 --> Arch
...
-   **コード先行の絶対禁止**: まず「要求」と「設計」をドキュメントで合意し、その後にコードを実装します。**特に、要求仕様書が作成された直後には、エージェントは自律的にアーキテクチャ設計へ進んではならず、必ず人間（マサ）の明示的な承認を待機します。**
-   **ドキュメントの絶対的最新化ルール (Single Source of Truth)**: 実装やテストの工程において機能追加やバグ修正による設計変更が必要と判断された場合、エージェントは「コードだけを先行して修正する」ことを固く禁じられています。必ず先立って要求定義（ステップ1）やアーキテクチャ設計（ステップ2）等の適切な工程へ戻り、ドキュメントを最新状態に更新した上で、再びコード実装・テストを進めます。**特に実装やテスト工程からの差し戻し、または人間の最終評価によるフィードバックによって要求内容に追加・変更・削除が生じた場合は、対応する要求仕様書を必ず直ちに書き換えて最新化しなければなりません（変更の必要がない部分はそのまま維持します）。**
...
-   **人間によるアーキテクチャ設計レビューの条件**: 「SW205_ソフトウェアアーキテクチャ設計書」の人間によるレビューは、**設計書が大幅に書き換わったときだけ**実施します。
    - **初回作成時**: 要求仕様書に基づいて最初に設計書を作成する際は、無い状態からの大幅な書き換えとなるため、必ず人間によるレビューを実施します。
    - **実装・テスト・評価を通じた改訂時**: マイナな設計変更や一部の修正であれば人間の再承認は不要とし、設計書が大幅に書き換わったときだけ人間による再レビューを行います。
